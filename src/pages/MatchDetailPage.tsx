...
import { getMatchAnalysis, createAnalysis } from '../api/analyses';
...
const MatchDetailPage: React.FC = () => {
    ...
    // matchId는 이제 Riot 매치 ID 문자열 (예: KR_7951942780)
    const riotMatchId = matchId || '';

    // 매치 상세 정보 조회
    const { data: matchDetail, isLoading: isLoadingDetail } = useQuery({
        queryKey: ['matchDetail', riotMatchId],
        queryFn: () => getMatchDetail(riotMatchId),
        enabled: !!riotMatchId,
    });
    ...
    const {
        ...
        queryKey: ['matchAnalysis', riotMatchId],
        queryFn: () => getMatchAnalysis(riotMatchId),
        enabled: !!riotMatchId,
        retry: false,
    });
    ...
    const { data: highlightsData, isLoading: isLoadingHighlights } = useQuery({
        queryKey: ['matchHighlights', riotMatchId],
        queryFn: () => getMatchHighlights(riotMatchId),
        enabled: !!riotMatchId,
    });

    // 분석 생성 뮤테이션
    const createAnalysisMutation = useMutation({
        mutationFn: () => createAnalysis(riotMatchId),
        onSuccess: () => {
            queryClient.invalidateQueries({ queryKey: ['matchAnalysis', riotMatchId] });
        },
    });
                        ...
                        <div>
                            ...
                            <p className="text-sm text-[#8B8B8B]">
                                ...
                                {matchDetail.gameVersion && (
                                    <span className="ml-2 text-xs text-[#5B5B5B]">패치 {matchDetail.gameVersion}</span>
                                )}
                            </p>
                        </div>
                        <div className={`
